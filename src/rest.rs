...
use crate::{
    ...
    output::{json_to_table, json_output},
};
...
impl Rest {
    ...
    pub async fn request(
        ...
        if body.is_some() {
            ...
            info!("curl -D - -s -X {} {} -H \"Content-Type:application/json\" -H \"X-Auth-Token:{}\"",
                  method.as_str(), url, self.token);
        }
        ...
        if body.is_some() {
            return Ok(builder.json(&body.unwrap()));
        }
        Ok(builder)
    }

    pub async fn post(&mut self, uri: &str, body: Value) -> anyhow::Result<Response> {
        self.request(reqwest::Method::POST, uri, Some(body)).await?.send_bench(true).await
    }

    pub async fn get(&mut self, uri: &str) -> anyhow::Result<Response> {
        self.request(reqwest::Method::GET, uri, None).await?.send_bench(true).await
    }
    ...
    pub async fn name_to_id(&mut self, uri: &str, name: &str) -> anyhow::Result<Uuid> {
        let body = ResourceBuilder::new()
            ...
            .build()?;
        let response: Vec<HashMap<String, Value>> = self.request(reqwest::Method::POST, uri, Some(body))
            .await?
            .send_bench(false)
            .await?
            .error_for_status()?
        ...
        match wanted.len() {
            ...
            1 => {
                Ok(Uuid::parse_str(wanted[0]["id"].as_str().unwrap())?)
            }
            _ => {
                ...
                if index >= wanted.len() {
                    Err(anyhow!("Your select {} is not in range 0-{}", index, wanted.len() - 1))
                } else {
...
pub async fn get_token(cfg: &config::Auth) -> anyhow::Result<String> {
    ...
    info!("curl -D - -s -X POST {} -H \"Content-Type:application/json\" -d '{}'", uri, body);
...
mod tests {
    ...
    async fn test_rest() -> Result<(), anyhow::Error> {
        ...
        let _response = api.request(Method::POST, String::from("neutron/network"), body).await?;
        Ok(())
    }
}
